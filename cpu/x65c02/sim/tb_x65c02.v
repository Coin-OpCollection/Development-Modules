...
//  x65c02_alu instance from the testbench so corner cases that are not
...
//=========================================================================
// DUT Instantiation:
// Complete x65c02 CPU under test. ALU is instantiated inside x65c02.
//=========================================================================
x65c02 uut (
    .clk   ( clk   ),
    .cen   ( cen   ),
...
// Independent x65c02_alu used by Sections 25 - 34 to drive the ALU
...
//=========================================================================
// ALU Operation Codes (mirror x65c02_alu.v):
...
initial begin
    $display("=============================================================");
    $display("  x65c02 Comprehensive Testbench");
    $display("  Reference: MOS Technology MCS6500 Hardware Manual");
    $display("             Rockwell R65C02 / R65C102 / R65C112 Datasheet");
    $display("=============================================================\n");
    ...
    //=====================================================================
    // SECTION 12: JMP Indirect (CMOS)
    // 6 cycles on R65C02 (NMOS was 5). The CMOS variant adds one
    // internal/dummy cycle and FIXES the NMOS page-wrap bug: the high
    // byte fetch uses a full 16-bit increment of the indirect pointer,
    // so JMP ($xxFF) reads the high byte from $(xx+1)00 not $xx00.
    //=====================================================================
    ...
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 6, "JMP ind timing (CMOS 6 cyc)");
    ...
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 6, "JMP ind timing 2 (CMOS 6 cyc)");
    check(uut.reg_pc, 16'h1744, "JMP ($0100): PC = $1744");

    // JMP ($00FF) - CMOS fix: no page-wrap. PC = {ram[$0100], ram[$00FF]}.
    ram[8'h00]  = 8'h13;                  // would have been used by NMOS bug
    ram[8'hFF]  = 8'h77;                  // low byte of target
    ram[9'h100] = 8'h44;                  // high byte of target (CMOS-correct)
    rom[12'h100] = 8'h6C;
    rom[12'h101] = 8'hFF;
    rom[12'h102] = 8'h00;
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 6, "JMP ($xxFF) timing (CMOS 6 cyc)");
    check(uut.reg_pc, 16'h4477, "JMP ($00FF) CMOS-fixed: PC = $4477");
    ...
    //=====================================================================
    // SECTION 37: CMOS BRA ($80) - Always Take
    // 3 cycles no cross, 4 cycles page cross. Branch is taken
    // unconditionally regardless of any P flag.
    //=====================================================================
    $display("\n=== SECTION 37: CMOS BRA ===\n");

    // BRA forward, no cross, all flags clear
    rom[12'h100] = 8'h80;
    rom[12'h101] = 8'h05;
    setup_test(16'h1100);
    uut.reg_p = 8'h24;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 3, "BRA fwd no-cross timing");
    check(uut.reg_pc, 16'h1107, "BRA +5: PC = $1107");

    // BRA forward, with all flags set (still taken)
    rom[12'h100] = 8'h80;
    rom[12'h101] = 8'h0A;
    setup_test(16'h1100);
    uut.reg_p = 8'hFF;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 3, "BRA flags-all-1 timing");
    check(uut.reg_pc, 16'h110C, "BRA +10 (flags=$FF): PC = $110C");

    // BRA backward, no cross
    rom[12'h110] = 8'h80;
    rom[12'h111] = 8'hFE;        // -2
    setup_test(16'h1110);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 3, "BRA back no-cross timing");
    check(uut.reg_pc, 16'h1110, "BRA -2: PC = $1110");

    // BRA forward with page cross
    rom[12'h0F0] = 8'h80;
    rom[12'h0F1] = 8'h20;
    setup_test(16'h10F0);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 4, "BRA fwd cross timing");
    check(uut.reg_pc, 16'h1112, "BRA +32 cross: PC = $1112");

    // BRA backward with page cross
    rom[12'h110] = 8'h80;
    rom[12'h111] = 8'hE0;        // -32
    setup_test(16'h1110);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 4, "BRA back cross timing");
    check(uut.reg_pc, 16'h10F2, "BRA -32 cross: PC = $10F2");

    //=====================================================================
    // SECTION 38: CMOS INC A / DEC A
    // 2 cycles. Operate on A, set N/Z, leave C/V untouched.
    //=====================================================================
    $display("\n=== SECTION 38: CMOS INC A / DEC A ===\n");
    ...
    uut.reg_a = 8'h0F;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 2, "INC A timing");
    check(uut.reg_a, 8'h10, "INC A: $0F -> $10");
    check(uut.reg_p[uut.FLAG_N], 1'b0, "INC A N=0");
    check(uut.reg_p[uut.FLAG_Z], 1'b0, "INC A Z=0");
    ...
    uut.reg_a = 8'hFF;
    run_instruction_timed(cycle_count);
    check(uut.reg_a, 8'h00, "INC A: $FF -> $00");
    check(uut.reg_p[uut.FLAG_Z], 1'b1, "INC A wrap Z=1");
    ...
    uut.reg_a = 8'h7F;
    run_instruction_timed(cycle_count);
    check(uut.reg_a, 8'h80, "INC A: $7F -> $80");
    check(uut.reg_p[uut.FLAG_N], 1'b1, "INC A neg N=1");
    ...
    uut.reg_a = 8'h10;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 2, "DEC A timing");
    check(uut.reg_a, 8'h0F, "DEC A: $10 -> $0F");
    ...
    uut.reg_a = 8'h01;
    run_instruction_timed(cycle_count);
    check(uut.reg_a, 8'h00, "DEC A: $01 -> $00");
    check(uut.reg_p[uut.FLAG_Z], 1'b1, "DEC A zero Z=1");
    ...
    uut.reg_a = 8'h00;
    run_instruction_timed(cycle_count);
    check(uut.reg_a, 8'hFF, "DEC A: $00 -> $FF");
    check(uut.reg_p[uut.FLAG_N], 1'b1, "DEC A wrap N=1");

    //=====================================================================
    // SECTION 39: CMOS PHX / PHY / PLX / PLY
    // PHX/PHY: 3 cycles. PLX/PLY: 4 cycles. PLX/PLY update N/Z from
    // the pulled value.
    //=====================================================================
    $display("\n=== SECTION 39: CMOS PHX/PHY/PLX/PLY ===\n");
    ...
    ram[9'h1FD] = 8'h00;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 3, "PHX timing");
    check({24'h0, ram[9'h1FD]}, 8'h5A, "PHX: stack[$01FD] = $5A");
    check(uut.reg_s, 8'hFC, "PHX: S = $FC");
    ...
    ram[9'h1FD] = 8'h00;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 3, "PHY timing");
    check({24'h0, ram[9'h1FD]}, 8'hA5, "PHY: stack[$01FD] = $A5");
    check(uut.reg_s, 8'hFC, "PHY: S = $FC");

    // PLX ($FA): negative value sets N
    rom[12'h100] = 8'hFA;
    setup_test(16'h1100);
    uut.reg_s = 8'hFB;
    ram[9'h1FC] = 8'h91;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 4, "PLX timing");
    check(uut.reg_x, 8'h91, "PLX: X = $91");
    check(uut.reg_s, 8'hFC, "PLX: S = $FC");
    check(uut.reg_p[uut.FLAG_N], 1'b1, "PLX N=1");
    check(uut.reg_p[uut.FLAG_Z], 1'b0, "PLX Z=0");

    // PLX: zero value sets Z
    rom[12'h100] = 8'hFA;
    setup_test(16'h1100);
    uut.reg_s = 8'hFB;
    ram[9'h1FC] = 8'h00;
    run_instruction_timed(cycle_count);
    check(uut.reg_x, 8'h00, "PLX zero: X = $00");
    check(uut.reg_p[uut.FLAG_Z], 1'b1, "PLX Z=1");
    ...
    ram[9'h1FC] = 8'h42;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 4, "PLY timing");
    check(uut.reg_y, 8'h42, "PLY: Y = $42");
    check(uut.reg_s, 8'hFC, "PLY: S = $FC");
    check(uut.reg_p[uut.FLAG_N], 1'b0, "PLY N=0");

    //=====================================================================
    // SECTION 40: CMOS STZ (Store Zero)
    // STZ zp/abs: 3/4 cycles. STZ zp,X/abs,X: 4/5 cycles.
    // Always writes $00 to the effective address. Flags unchanged.
    //=====================================================================
    $display("\n=== SECTION 40: CMOS STZ ===\n");
    ...
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 3, "STZ zp timing");
    check({24'h0, ram[8'h40]}, 8'h00, "STZ zp: ram[$40] = $00");
    ...
    uut.reg_x = 8'h05;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 4, "STZ zp,X timing");
    check({24'h0, ram[8'h45]}, 8'h00, "STZ zp,X: ram[$45] = $00");
    ...
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 4, "STZ abs timing");
    check({24'h0, ram[9'h080]}, 8'h00, "STZ abs: ram[$0080] = $00");

    // STZ abs,X ($9E) - no cross
    rom[12'h100] = 8'h9E;
    rom[12'h101] = 8'h80;
    rom[12'h102] = 8'h00;
    ram[9'h083] = 8'hFF;
    ...
    uut.reg_x = 8'h03;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 5, "STZ abs,X timing");
    check({24'h0, ram[9'h083]}, 8'h00, "STZ abs,X: ram[$0083] = $00");

    // STZ does not change flags
    rom[12'h100] = 8'h64;
    rom[12'h101] = 8'h40;
    setup_test(16'h1100);
    uut.reg_p = 8'hA5;
    run_instruction_timed(cycle_count);
    check(uut.reg_p, 8'hA5, "STZ preserves P");

    //=====================================================================
    // SECTION 41: CMOS TRB / TSB
    // TRB zp/abs: 5/6 cycles. TSB zp/abs: 5/6 cycles. Update Z only,
    // Z = (M & A == 0). TRB writes M & ~A, TSB writes M | A.
    //=====================================================================
    $display("\n=== SECTION 41: CMOS TRB / TSB ===\n");
    ...
    uut.reg_a = 8'h0F;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 5, "TRB zp timing");
    check({24'h0, ram[8'h50]}, 8'hF0, "TRB zp: $FF & ~$0F = $F0");
    check(uut.reg_p[uut.FLAG_Z], 1'b0, "TRB zp Z=0 (FF & 0F != 0)");

    // TRB zp: zero result sets Z
    rom[12'h100] = 8'h14;
    rom[12'h101] = 8'h50;
    ram[8'h50] = 8'h0F;
    setup_test(16'h1100);
    uut.reg_a = 8'hF0;
    run_instruction_timed(cycle_count);
    check({24'h0, ram[8'h50]}, 8'h0F, "TRB zp no-clear: $0F & ~$F0 = $0F");
    check(uut.reg_p[uut.FLAG_Z], 1'b1, "TRB zp Z=1 ($0F & $F0 == 0)");
    ...
    uut.reg_a = 8'h0A;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 6, "TRB abs timing");
    check({24'h0, ram[9'h080]}, 8'hA0, "TRB abs: $AA & ~$0A = $A0");
    ...
    uut.reg_a = 8'hF0;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 5, "TSB zp timing");
    check({24'h0, ram[8'h60]}, 8'hFF, "TSB zp: $0F | $F0 = $FF");
    check(uut.reg_p[uut.FLAG_Z], 1'b1, "TSB zp Z=1 ($0F & $F0 == 0)");

    // TSB zp: non-zero AND clears Z
    rom[12'h100] = 8'h04;
    rom[12'h101] = 8'h60;
    ram[8'h60] = 8'h0F;
    setup_test(16'h1100);
    uut.reg_a = 8'h08;
    run_instruction_timed(cycle_count);
    check({24'h0, ram[8'h60]}, 8'h0F, "TSB zp no-set: $0F | $08 = $0F");
    check(uut.reg_p[uut.FLAG_Z], 1'b0, "TSB zp Z=0 ($0F & $08 != 0)");
    ...
    uut.reg_a = 8'hC0;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 6, "TSB abs timing");
    check({24'h0, ram[9'h080]}, 8'hD2, "TSB abs: $12 | $C0 = $D2");

    //=====================================================================
    // SECTION 42: CMOS BIT Extensions
    // BIT # ($89): 2 cycles, only Z affected (preserves N and V).
    // BIT zp,X ($34): 4 cycles, full BIT (N=M[7], V=M[6], Z=M&A==0).
    // BIT abs,X ($3C): 4 cycles no cross, 5 with cross, full BIT.
    //=====================================================================
    $display("\n=== SECTION 42: CMOS BIT Extensions ===\n");

    // BIT # ($89): preserves N and V from existing P
    rom[12'h100] = 8'h89;
    rom[12'h101] = 8'h0F;
    setup_test(16'h1100);
    uut.reg_a = 8'hF0;
    uut.reg_p = 8'h24;            // N=0, V=0
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 2, "BIT # timing");
    check(uut.reg_p[uut.FLAG_Z], 1'b1, "BIT # Z=1 ($F0 & $0F == 0)");
    check(uut.reg_p[uut.FLAG_N], 1'b0, "BIT # N preserved (0)");
    check(uut.reg_p[uut.FLAG_V], 1'b0, "BIT # V preserved (0)");

    // BIT # with N=1,V=1 set in P -> still preserved
    rom[12'h100] = 8'h89;
    rom[12'h101] = 8'h08;
    setup_test(16'h1100);
    uut.reg_a = 8'h0F;
    uut.reg_p = 8'hE4;            // N=1, V=1
    run_instruction_timed(cycle_count);
    check(uut.reg_p[uut.FLAG_Z], 1'b0, "BIT # Z=0 ($0F & $08 != 0)");
    check(uut.reg_p[uut.FLAG_N], 1'b1, "BIT # N preserved (1)");
    check(uut.reg_p[uut.FLAG_V], 1'b1, "BIT # V preserved (1)");

    // BIT zp,X ($34): full flags
    rom[12'h100] = 8'h34;
    rom[12'h101] = 8'h60;
    ram[8'h65] = 8'hC0;           // N=1, V=1
    setup_test(16'h1100);
    uut.reg_a = 8'h0F;
    uut.reg_x = 8'h05;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 4, "BIT zp,X timing");
    check(uut.reg_p[uut.FLAG_N], 1'b1, "BIT zp,X N=1 (M[7])");
    check(uut.reg_p[uut.FLAG_V], 1'b1, "BIT zp,X V=1 (M[6])");
    check(uut.reg_p[uut.FLAG_Z], 1'b1, "BIT zp,X Z=1 ($0F & $C0 == 0)");

    // BIT abs,X ($3C): full flags, no page cross
    rom[12'h100] = 8'h3C;
    rom[12'h101] = 8'h80;
    rom[12'h102] = 8'h00;
    ram[9'h083] = 8'h81;          // N=1, V=0
    setup_test(16'h1100);
    uut.reg_a = 8'h81;
    uut.reg_x = 8'h03;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 4, "BIT abs,X no-cross timing");
    check(uut.reg_p[uut.FLAG_N], 1'b1, "BIT abs,X N=1");
    check(uut.reg_p[uut.FLAG_V], 1'b0, "BIT abs,X V=0");
    check(uut.reg_p[uut.FLAG_Z], 1'b0, "BIT abs,X Z=0 ($81 & $81 != 0)");

    //=====================================================================
    // SECTION 43: CMOS JMP (abs,X)
    // 6 cycles. PC = mem[(base + X)] | (mem[(base + X) + 1] << 8)
    // with full 16-bit pointer increment (no page-wrap bug).
    //=====================================================================
    $display("\n=== SECTION 43: CMOS JMP (abs,X) ===\n");
    ...
    uut.reg_x = 8'h00;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 6, "JMP (abs,X) timing");
    check(uut.reg_pc, 16'h1733, "JMP ($1500,X=0): PC = $1733");

    // JMP ($1500,X) with X=4 -> pointer at $1504
    rom[12'h504] = 8'h66;
    rom[12'h505] = 8'h18;
    rom[12'h100] = 8'h7C;
    rom[12'h101] = 8'h00;
    ...
    uut.reg_x = 8'h04;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 6, "JMP (abs,X) X=4 timing");
    check(uut.reg_pc, 16'h1866, "JMP ($1500,X=4): PC = $1866");

    // JMP (abs,X) page-cross fix: base = $15FE, X=2 -> pointer = $1600.
    // Read low at $1600, high at $1601 (full 16-bit increment).
    rom[12'h600] = 8'h99;
    rom[12'h601] = 8'h19;
    rom[12'h100] = 8'h7C;
    rom[12'h101] = 8'hFE;
    ...
    uut.reg_x = 8'h02;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 6, "JMP (abs,X) page-cross timing");
    check(uut.reg_pc, 16'h1999, "JMP ($15FE,X=2) cross: PC = $1999");

    //=====================================================================
    // SECTION 44: CMOS (zp) Indirect
    // 5 cycles. Effective address = mem[zp] | (mem[zp+1] << 8) with
    // 8-bit ZP wrap on the pointer increment.
    //=====================================================================
    $display("\n=== SECTION 44: CMOS (zp) Indirect ===\n");

    // Pre-populate pointer at $0050 -> $01F0 (within RAM range)
    ram[8'h50]  = 8'hF0;
    ram[8'h51]  = 8'h01;
    ram[9'h1F0] = 8'h77;          // target data
    ...
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 5, "LDA (zp) timing");
    check(uut.reg_a, 8'h77, "LDA ($50): A = $77");
    ...
    uut.reg_a = 8'hAB;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 5, "STA (zp) timing");
    check({24'h0, ram[9'h1F0]}, 8'hAB, "STA ($50): ram[$01F0] = $AB");
    ...
    uut.reg_a = 8'hF0;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 5, "ORA (zp) timing");
    check(uut.reg_a, 8'hFF, "ORA ($50): A = $FF");
    ...
    uut.reg_a = 8'hF3;
    run_instruction_timed(cycle_count);
    check(uut.reg_a, 8'h03, "AND ($50): A = $03");
    ...
    uut.reg_a = 8'hAA;
    run_instruction_timed(cycle_count);
    check(uut.reg_a, 8'h55, "EOR ($50): A = $55");
    ...
    uut.reg_p = 8'h24;
    run_instruction_timed(cycle_count);
    check(uut.reg_a, 8'h30, "ADC ($50): A = $30");
    ...
    uut.reg_a = 8'h44;
    run_instruction_timed(cycle_count);
    check(uut.reg_p[uut.FLAG_Z], 1'b1, "CMP ($50): equal Z=1");
    check(uut.reg_p[uut.FLAG_C], 1'b1, "CMP ($50): equal C=1");
    ...
    setup_test(16'h1100);
    uut.reg_a = 8'h30;
    uut.reg_p = 8'h25;            // C=1 (no borrow)
    run_instruction_timed(cycle_count);
    check(uut.reg_a, 8'h20, "SBC ($50): A = $20");

    // (zp) ZP wrap: pointer at $00FF -> low at $00FF, high at $0000
    ram[8'hFF]  = 8'h78;
    ram[8'h00]  = 8'h01;          // wraps -> high byte $01
    ram[9'h178] = 8'h99;
    rom[12'h100] = 8'hB2;
    rom[12'h101] = 8'hFF;
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check(uut.reg_a, 8'h99, "LDA ($FF) ZP wrap: A = $99");

    //=====================================================================
    // SECTION 45: CMOS RMB / SMB (Rockwell)
    // 5 cycles. RMB n,zp clears bit n; SMB n,zp sets bit n. No flag
    // updates. Bit number encoded in opcode high nibble (ir[6:4]).
    //=====================================================================
    $display("\n=== SECTION 45: CMOS RMB / SMB ===\n");
    ...
    uut.reg_p = 8'hA5;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 5, "RMB0 timing");
    check({24'h0, ram[8'h70]}, 8'hFE, "RMB0: $FF -> $FE");
    check(uut.reg_p, 8'hA5, "RMB0 preserves P");
    ...
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check({24'h0, ram[8'h70]}, 8'hF7, "RMB3: $FF -> $F7");
    ...
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check({24'h0, ram[8'h70]}, 8'h7F, "RMB7: $FF -> $7F");

    // RMB on already-clear bit (no change)
    rom[12'h100] = 8'h17;        // RMB1
    rom[12'h101] = 8'h70;
    ram[8'h70] = 8'hF0;
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check({24'h0, ram[8'h70]}, 8'hF0, "RMB1 already-clear: $F0 -> $F0");
    ...
    uut.reg_p = 8'hA5;
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 5, "SMB0 timing");
    check({24'h0, ram[8'h71]}, 8'h01, "SMB0: $00 -> $01");
    check(uut.reg_p, 8'hA5, "SMB0 preserves P");
    ...
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check({24'h0, ram[8'h71]}, 8'h10, "SMB4: $00 -> $10");
    ...
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check({24'h0, ram[8'h71]}, 8'h80, "SMB7: $00 -> $80");

    // SMB on already-set bit (no change)
    rom[12'h100] = 8'h97;        // SMB1
    rom[12'h101] = 8'h71;
    ram[8'h71] = 8'h0F;
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check({24'h0, ram[8'h71]}, 8'h0F, "SMB1 already-set: $0F -> $0F");

    // Sweep all 8 RMB bits and verify masks
    for(i = 0; i < 8; i = i + 1) begin
        rom[12'h100] = 8'h07 | (i << 4);
        rom[12'h101] = 8'h72;
        ram[8'h72] = 8'hFF;
        setup_test(16'h1100);
        run_instruction_timed(cycle_count);
        check({24'h0, ram[8'h72]}, {24'h0, 8'hFF & ~(8'h01 << i)},
              "RMBn sweep: bit cleared correctly");
    end

    // Sweep all 8 SMB bits
    for(i = 0; i < 8; i = i + 1) begin
        rom[12'h100] = 8'h87 | (i << 4);
        rom[12'h101] = 8'h73;
        ram[8'h73] = 8'h00;
        setup_test(16'h1100);
        run_instruction_timed(cycle_count);
        check({24'h0, ram[8'h73]}, {24'h0, 8'h01 << i},
              "SMBn sweep: bit set correctly");
    end

    //=====================================================================
    // SECTION 46: CMOS BBR / BBS (Rockwell bit branch)
    // 5 cycles not taken, 6 cycles taken, 7 cycles taken with page cross.
    // 3-byte instruction: opcode + zp + signed offset.
    //=====================================================================
    $display("\n=== SECTION 46: CMOS BBR / BBS ===\n");

    // BBR0 ($0F): branch if zp bit 0 = 0. Bit clear -> taken
    rom[12'h100] = 8'h0F;
    rom[12'h101] = 8'h80;
    rom[12'h102] = 8'h05;
    ram[8'h80] = 8'hFE;          // bit 0 = 0
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 6, "BBR0 taken timing");
    check(uut.reg_pc, 16'h1108, "BBR0 bit clear: branch taken");

    // BBR0: bit set -> not taken (PC advances 3)
    rom[12'h100] = 8'h0F;
    rom[12'h101] = 8'h80;
    rom[12'h102] = 8'h05;
    ram[8'h80] = 8'hFF;          // bit 0 = 1
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 5, "BBR0 not-taken timing");
    check(uut.reg_pc, 16'h1103, "BBR0 bit set: PC = $1103");

    // BBS7 ($FF): branch if zp bit 7 = 1
    rom[12'h100] = 8'hFF;
    rom[12'h101] = 8'h80;
    rom[12'h102] = 8'h08;
    ram[8'h80] = 8'h80;          // bit 7 = 1
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 6, "BBS7 taken timing");
    check(uut.reg_pc, 16'h110B, "BBS7 bit set: PC = $110B");

    // BBS7 not taken
    rom[12'h100] = 8'hFF;
    rom[12'h101] = 8'h80;
    rom[12'h102] = 8'h08;
    ram[8'h80] = 8'h7F;          // bit 7 = 0
    setup_test(16'h1100);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 5, "BBS7 not-taken timing");
    check(uut.reg_pc, 16'h1103, "BBS7 bit clear: PC = $1103");

    // BBR taken with page cross (forward)
    rom[12'h0F0] = 8'h0F;        // BBR0
    rom[12'h0F1] = 8'h81;
    rom[12'h0F2] = 8'h20;        // +32
    ram[8'h81] = 8'h00;          // bit 0 = 0 -> taken
    setup_test(16'h10F0);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 7, "BBR fwd cross timing");
    check(uut.reg_pc, 16'h1113, "BBR fwd cross: PC = $1113");

    // BBS taken backward
    rom[12'h120] = 8'h8F;        // BBS0
    rom[12'h121] = 8'h82;
    rom[12'h122] = 8'hFB;        // -5
    ram[8'h82] = 8'h01;          // bit 0 = 1 -> taken
    setup_test(16'h1120);
    run_instruction_timed(cycle_count);
    check_timing(cycle_count, 6, "BBS back no-cross timing");
    check(uut.reg_pc, 16'h111E, "BBS -5: PC = $111E");

    // Sweep all 8 BBR bits with bit clear (all should branch)
    for(i = 0; i < 8; i = i + 1) begin
        rom[12'h100] = 8'h0F | (i << 4);
        rom[12'h101] = 8'h83;
        rom[12'h102] = 8'h00;
        ram[8'h83] = 8'h00;          // all bits clear
        setup_test(16'h1100);
        run_instruction_timed(cycle_count);
        check(uut.reg_pc, 16'h1103, "BBRn sweep all clear: PC = $1103");
    end

    // Sweep all 8 BBS bits with bit set (all should branch)
    for(i = 0; i < 8; i = i + 1) begin
        rom[12'h100] = 8'h8F | (i << 4);
        rom[12'h101] = 8'h84;
        rom[12'h102] = 8'h00;
        ram[8'h84] = 8'hFF;          // all bits set
        setup_test(16'h1100);
        run_instruction_timed(cycle_count);
        check(uut.reg_pc, 16'h1103, "BBSn sweep all set: PC = $1103");
    end

    //=====================================================================
    // SECTION 47: CMOS BCD N/V/Z Fix
    // R65C02 fixes the NMOS BCD bug: N, V, Z flags reflect the BCD
    // result (not the binary result) after ADC/SBC in decimal mode.
    //=====================================================================
    $display("\n=== SECTION 47: CMOS BCD N/V/Z Fix ===\n");

    // Direct ALU: ADC BCD with result that produces decimal Z=1
    apply_alu(8'h25, 8'h75, ALU_ADC, 8'h08);   // D=1, C=0, A+B=$00 BCD
    check_alu_q(8'h00, "ADC BCD 25+75 = 00 BCD");
    check_alu_flag(1, 1'b1, "ADC BCD Z=1 (decimal $00)");
    check_alu_flag(0, 1'b1, "ADC BCD C=1 (decimal carry)");

    // Direct ALU: ADC BCD with N=1 (decimal $80+)
    apply_alu(8'h45, 8'h45, ALU_ADC, 8'h08);   // D=1, 45+45 = 90 BCD
    check_alu_q(8'h90, "ADC BCD 45+45 = 90 BCD");
    check_alu_flag(7, 1'b1, "ADC BCD N=1 (high nibble 9)");

    // Direct ALU: SBC BCD with Z=1 (decimal $00)
    apply_alu(8'h50, 8'h50, ALU_SBC, 8'h09);   // D=1, C=1 (no borrow), 50-50=00
    check_alu_q(8'h00, "SBC BCD 50-50 = 00 BCD");
    check_alu_flag(1, 1'b1, "SBC BCD Z=1 (decimal $00)");

    // Direct ALU: SBC BCD with N=1
    apply_alu(8'h00, 8'h01, ALU_SBC, 8'h09);   // D=1, C=1, 00-01=99 BCD (borrow)
    check_alu_q(8'h99, "SBC BCD 00-01 = 99 BCD (borrow)");
    check_alu_flag(7, 1'b1, "SBC BCD N=1 (high nibble 9)");

    //=====================================================================
    // Final Test Summary
    //=====================================================================
    $display("\n=============================================================");
    $display("  x65c02 Test Results");
    $display("=============================================================");
    ...
    if(failed_tests == 0) begin
        ...
        $display("  Cycle-accurate to Rockwell R65C02 Datasheet");
    end else begin
